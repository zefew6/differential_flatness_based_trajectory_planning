...
class TrajectoryOptimizer:
    """Low-level MINCO trajectory optimizer.

    This class owns only the numerical optimization state:
    L-BFGS variable packing, MINCO coefficient generation, cost-term
    accumulation, and gradient back-propagation to waypoints/times.
    Path search, path resampling, and corridor construction live in
    :mod:`m0.minco_planner.planner`.
    """
    ...
    def set_sfc_corridors(self, hPolys_per_traj: list):
        """设置 SFC 走廊数据（SFC 方法）。
    ...
    def set_params(self, wei_time=None, wei_feas=None, mini_T=None,
                 max_vel=None, max_acc=None,
        ...
        if max_vel is not None:
            ...
            for c in getattr(self, 'feasConstr_container', []):
                c.max_vel = float(max_vel)
        if max_acc is not None:
            self.max_acc = float(max_acc)
            for c in getattr(self, 'feasConstr_container', []):
                c.max_acc = float(max_acc)
    ...
    def set_debug_print_every(self, n: int):
    ...
    def optimize(self, iniStates, finStates, initInnerPts, initTs, initSegTs=None):
        """优化轨迹
        ...
        for i in range(self.trajnum):
            clipped_ini = self.clip_state_constraints(iniStates[i], self.max_vel, self.max_acc)
            clipped_fin = self.clip_state_constraints(finStates[i], self.max_vel, self.max_acc)
        ...
        for i in range(self.trajnum):
            ...
            self.real_time_to_virtual(segment_times, VT)
    ...
    def _cost_function_callback(self, x):
        ...
        for trajid in range(self.trajnum):
            ...
            self.virtual_time_to_real(t_seg, T_seg)
        ...
        for trajid in range(self.trajnum):
            ...
            for seg_id in range(piece_num):
                gradt_seg, time_cost_seg = self.virtual_time_grad_cost(
    ...
    def real_time_to_virtual(self, RT, VT):
        """真实时间转虚拟时间（无约束优化）"""
    ...
    def virtual_time_to_real(self, VT, RT):
        """虚拟时间转真实时间"""
    ...
    def virtual_time_grad_cost(self, RT, VT, gdRT):
        """计算虚拟时间的梯度和时间代价
    ...
    def clip_state_constraints(self, state, max_vel, max_acc):
        """
        裁剪状态约束（用于初始/终止状态）
    ...
    def get_optimized_trajectories(self):
        """获取优化后的轨迹"""
        return self.jerkOpt_container

    # Backward-compatible method names used by old examples/callers.
    setGridMap = set_grid_map
    setSFCCorridors = set_sfc_corridors
    setParam = set_params
    setDebugPrintEvery = set_debug_print_every
    OptimizeTrajectory = optimize
    RealT2VirtualT = real_time_to_virtual
    VirtualT2RealT = virtual_time_to_real
    VirtualTGradCost = virtual_time_grad_cost
    clipStateConstraints = clip_state_constraints
    getOptimizedTrajectories = get_optimized_trajectories
