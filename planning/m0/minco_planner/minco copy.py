"""
MINCO 最小实现 - 用于 s=3 (三次多项式) 的最小控制轨迹优化
...
class MINCO:
    """
    MINCO for s=3 (五次多项式) 和非均匀时间 - 二维版本
    
    每段轨迹是三次多项式: p(t) = c0 + c1*t + c2*t^2 + c3*t^3 + c4*t^4 + c5*t^5 
    确保位置、速度、加速度连续 (C3 连续)
    """
    ...
    def __init__(self, head_pva, tail_pva, waypoints, durations):
        ...
        参数:
            head_pv: 起始状态 [pos, vel], shape (3, 2) - [[px,py], [vx,vy], [ax,ay]]
            tail_pv: 终止状态 [pos, vel], shape (3, 2)
        ...
        self.T4 = self.T ** 4
        self.T5 = self.T ** 5
        # print(self.T,self.T2,self.T3)
        # 求解系数
        # T= time.time()
        self.coeffs = self._solve() # 2Ms * m
        # print((time.time() - T)*1000)
    
    def _solve(self):
        """构造并求解线性系统 Ax = b"""
        #2Ms*m 维，其中M为段数 s为控制量阶数（3阶），m为轨迹个数（x和y）
        s = self.s
        m = self.m
        M = self.N
        n = 2 * M * s # 总方程数 = 2Ms = 2 * 段数 * 阶数
        A = np.zeros((n, n)) # 2Ms * 2Ms
        # 构造右侧向量 b
        b = np.zeros((n, m)) # 2Ms * m
